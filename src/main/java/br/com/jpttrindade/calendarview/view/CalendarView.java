...
import br.com.jpttrindade.calendarview.snaphelper.SnapHelperOneByOne;
...
public class CalendarView extends FrameLayout implements onDateChangedListener, onMonthChangedListener {
    ...
    private OnDayChacedListener onDayChacedListener;
    ...
    private void init(AttributeSet attrs, int defStyle) {
        ...
        LinearSnapHelper snapHelperOneByOne = new SnapHelperOneByOne();
        rl_calendar.setLayoutManager(mLayoutManager);
        snapHelperOneByOne.attachToRecyclerView(rl_calendar);
            ...
            public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
               ...
               /* visibleItemCount = recyclerView.getChildCount();
                ...
                if (firstVisibleItem != tempFirstVisibleItem) {
                    ...
                    tvMonth.setText(monthsList.get(month.value - 1));
                    tvYear.setText(String.valueOf(month.year));
                    onDayChacedListener.onChanged(month.value, month.year, month.lastDay);
                    changeMonthColor(month.value - 1, month.year);
...
//        a.recycle();
    ...
    private void changeMonthColor(int month, int year) {
        try {
            ...
            if (currentDate.compareTo(selectedDate) == 0) {
                tvMonth.setTextColor(ContextCompat.getColor(mContext, R.color.font2));
            } else {
                tvMonth.setTextColor(ContextCompat.getColor(mContext, R.color.black));
            }
...
//        if (TypedValue.TYPE_REFERENCE == a.getType(R.styleable.CalendarView_weekdayNameBackgroundColor)) {
            ...
            calendarAttrs.daytextsize = (float) a.getDimension(R.styleable.CalendarView_dayTextSize, TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, (float) 14, displayMetrics));
            calendarAttrs.dayspace = (int) a.getDimension(R.styleable.CalendarView_daySpace, TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, (float) 24, displayMetrics));
            ...
            calendarAttrs.eventCircleWidht = a.getDimension(R.styleable.CalendarView_eventCircleWidth, TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, (float) 14, displayMetrics));
            calendarAttrs.eventCircleHeight = a.getDimension(R.styleable.CalendarView_eventCircleHeight, TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, (float) 14, displayMetrics));
    ...
    public void setOnDayChanged(OnDayChacedListener onDayChacedListener) {
        this.onDayChacedListener = onDayChacedListener;
    }


    public void addEvent(int day, int month, int year, LinkedHashSet<String> events) {
        mCalendarAdapter.addEvent(day, month, year, events);
    }

    public LinkedHashSet<String> getEventsOnParticulerDate(int day, int month, int year) {
        return mCalendarAdapter.hasEventColor(day, month, year);
    }
    ...
    public void onDateChanged(String year) {
        tvYear.setText(year);
        moveToMonth(year, tvMonth.getText().toString());
        for (String mon : monthsList) {
            if (mon.toLowerCase().equals(tvMonth.getText().toString().toLowerCase())) {
                changeMonthColor(monthsList.indexOf(mon) + 1, Integer.parseInt(tvYear.getText().toString()));
            }
        }
        //tvMonth.setText("");
    }
    ...
    @Override
    public void onMonthChanged(final String month) {
        moveToMonth(tvYear.getText().toString(), month);
        for (String mon : monthsList) {
    ...
    public void moveToMonth(String year, String month) {
        try {
            ...
            for (String mon : monthsList) {
                if (mon.toLowerCase().equals(month.toLowerCase())) {
                    mCalendarAdapter.reloadCalendar(Integer.parseInt(year), monthsList.indexOf(mon) + 1, true);
                    movetoSelectMonth(mCalendarAdapter.mMonths, year, monthsList.indexOf(mon) + 1);
    ...
    /*------------Moving the Dynamic Created Position--------------*/
    private void movetoSelectMonth(final ArrayList<Month> mMonths, final String selectYear, final int selectMonth) {
        try {
            new Handler().postDelayed(new Runnable() {
                @Override
                public void run() {
                    for (Month month : mMonths) {
                        if (selectYear.equals(String.valueOf(month.year)) && selectMonth == month.value) {
                            int position = mMonths.indexOf(month);
                            rl_calendar.scrollToPosition(position);
                            onDayChacedListener.onChanged(mMonths.get(position).value, mMonths.get(position).year, mMonths.get(position).lastDay);
                            return;
                        }
    ...
    public void clearEvents() {
        try {
            mCalendarAdapter.mEventsColor.clear();
            mCalendarAdapter.mEvents.clear();
        } catch (Exception ex) {
            ex.printStackTrace();
        }
    ...
    public interface OnDayChacedListener {
        public void onChanged(int month, int year, int lastDayOfMonth);
    }
    ...
    public class Attributes {
        ...
        public float eventCircleWidht;
        public float eventCircleHeight;
    }
